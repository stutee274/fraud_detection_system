...
# app_integrated_fixed.py - Fixed CORS and Deployment Issues
"""
✅ FIXED: Proper CORS configuration
✅ FIXED: Preflight OPTIONS handling
✅ FIXED: Cross-origin headers
✅ Dual Mode: Banking + Credit Card
✅ Database: PostgreSQL with prediction tracking
✅ Feedback: User feedback collection
✅ Analytics: Dashboard statistics
...
print("="*80)
print(" INTEGRATED FRAUD DETECTION SYSTEM - FIXED VERSION")
print("="*80)
...
def home():
    return jsonify({
        "status": "healthy",
        "service": "Integrated Fraud Detection System - FIXED",
        "version": "2.0",
        "cors": "enabled",
        "modes": {
            "banking": {
    ...
    try:
        ...
        if mode == 'credit_card':
            ...
            if not model_cc:
                return jsonify({
                    "status": "error", 
                    "message": "Credit card model unavailable"
                }), 400
            ...
            if not model_banking:
                return jsonify({
                    "status": "error", 
                    "message": "Banking model unavailable"
                }), 400
...
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 5000))
    print("\n🚀 Starting Integrated Fraud Detection Server (FIXED)...")
    print(f"📍 Port: {port}")
