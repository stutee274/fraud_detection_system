...
# =====================================================
# ✅ FIXED CORS CONFIGURATION (PRODUCTION SAFE)
# =====================================================
CORS(
    app,
    resources={
...
@app.route("/api/check-fraud", methods=["OPTIONS"])
def preflight_check_fraud():
    return "", 204
...
try:
    ...
    try:
        ...
        print("⚠️  BANKING Model: Scaler not found")

    model_banking = temp_model
    print(f"✅ BANKING Model loaded ({len(features_banking)} features)")
...
try:
    ...
    if os.path.exists("models/model_config.json"):
        with open("models/model_config.json") as f:
            threshold_cc = json.load(f).get("default_threshold", 0.4)

    model_cc = temp_model
    print(f"✅ CREDIT CARD Model loaded ({len(features_cc)} features)")

except Exception as e:
    print(f"⚠️  CREDIT CARD Model not available: {e}")
...
try:
    from database.db_dual import (
        init_database, close_database, save_prediction_to_db,
        get_prediction_by_id, get_recent_predictions,
        update_prediction_feedback
    )

    DB_ENABLED = init_database()
    print("✅ Database connected" if DB_ENABLED else "⚠️ Database disabled")
...
def prepare_banking_features(data):
    ...
    final_df = pd.DataFrame()
    for col in features_banking:
        final_df[col] = df[col] if col in df else 0
...
def predict():
    try:
        ...
        if mode == "credit_card":
            if not model_cc:
                return jsonify({"error": "Credit card model unavailable"}), 400

            X = prepare_credit_card_features(data)
            proba = model_cc.predict_proba(X)[0][1]
            pred = int(proba >= threshold_cc)
            amount = float(data.get("Amount", 0))
            threshold = threshold_cc
            ...
            if not model_banking:
                return jsonify({"error": "Banking model unavailable"}), 400

            X = prepare_banking_features(data)
            proba = model_banking.predict_proba(X)[0][1]
            pred = int(proba >= threshold_banking)
            amount = float(data.get("Transaction_Amount", 0))
            threshold = threshold_banking
        ...
        response = {
            ...
            "threshold": threshold,
            "transaction_amount": amount,
            "message": "FRAUD" if pred else "NORMAL"
        }
